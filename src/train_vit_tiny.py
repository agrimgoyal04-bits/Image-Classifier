...
Same as train.py but using:
- Backbone: vit_tiny_patch16_224
- Separate output dir: outputs_vit_tiny
"""
...
BACKBONE_NAME = "vit_tiny_patch16_224"      # <-- changed
...
OUTPUT_DIR = "outputs_vit_tiny"             # <-- changed so it doesn't overwrite
...
def tsne_plot(features, labels, idx_to_class, out_path):
    ...
    plt.title("t-SNE of ViT Tiny Features (val set)")
    plt.tight_layout()
    plt.savefig(out_path, dpi=200)
    plt.close()
            ...
            if preds_class[i].item() != y_class[i].item():
                try:
                    if isinstance(rows, (list, tuple)):
                        ...
                        row_dict = rows
                    img_path = (
                        row_dict.get("image_path", "unknown")
...
def main():
    ...
    if os.path.exists(best_model_path):
        print(
            f"Found existing best model at {best_model_path}. "
            "Skipping training and going straight to evaluation."
        )
    else:
